...
function formatTickaCompactDate(date: string) {
...
function shouldUseTickaFixtures() {
  return parseBooleanEnv(process.env.USE_TICKA_FIXTURES);
}
...
function isEmissioniFixtureRow(value: unknown): value is TickaEmissioniApiRow {
...
function extractFixtureRows(payload: unknown): TickaEmissioniApiRow[] {
  if (Array.isArray(payload)) {
    return payload.filter(isEmissioniFixtureRow);
  }
  ...
  if (Array.isArray(record.result)) {
    return record.result.filter(isEmissioniFixtureRow);
  }

  if (Array.isArray(record.rows)) {
    return record.rows.filter(isEmissioniFixtureRow);
  }

  if (record.payload && typeof record.payload === "object") {
    const payloadRows = extractFixtureRows(record.payload);
...
async function walkForFixtureCandidates(rootDir: string, depth = 0): Promise<string[]> {
  if (depth > 4) {
    return [];
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      results.push(...(await walkForFixtureCandidates(fullPath, depth + 1)));
      continue;
    }
    ...
    if (
      lowerName.includes("emissioni") ||
      lowerName.includes("report") ||
      lowerName.includes("ticka")
    ) {
...
let cachedFixtureRowsPromise: Promise<{ filePath: string | null; rows: TickaEmissioniApiRow[] }> | null = null;

async function loadTickaEmissioniFixtureRows() {
  if (!cachedFixtureRowsPromise) {
    cachedFixtureRowsPromise = (async () => {
      const cwd = process.cwd();
      const envPath = process.env.TICKA_EMISSIONI_FIXTURE_FILE?.trim();
      const explicitCandidates = [
        envPath ? path.resolve(cwd, envPath) : null,
        path.join(cwd, "ticka-report-emissioni.json"),
        path.join(cwd, "report-emissioni.json"),
        path.join(cwd, "fixtures", "ticka", "report-emissioni.json"),
        path.join(cwd, "fixtures", "ticka", "ReportEmissioni.json"),
        path.join(cwd, "data", "ticka", "report-emissioni.json"),
        path.join(cwd, "data", "ticka", "ReportEmissioni.json"),
      ].filter((candidate): candidate is string => Boolean(candidate));

      const recursiveCandidates = await walkForFixtureCandidates(cwd);
      const candidates = Array.from(new Set([...explicitCandidates, ...recursiveCandidates]));
      ...
      for (const candidate of candidates) {
        ...
        try {
          ...
          const rows = extractFixtureRows(parsed);

          if (rows.length > 0) {
            console.log("[ticka-emissioni] fixture file loaded", {
              fixtureFileUsed: candidate,
              rawRowCount: rows.length,
            });
            if (isDebugTickaEnabled()) {
              console.log("[ticka-emissioni] fixture file sample", {
                fixtureFileUsed: candidate,
          ...
          console.warn("[ticka-emissioni] fixture candidate unreadable", {
            fixtureFileUsed: candidate,
      ...
      console.warn("[ticka-emissioni] no local ReportEmissioni fixture found", {
        searchedCandidates: candidates.map((candidate) => path.relative(cwd, candidate)),
      });
  ...
  return cachedFixtureRowsPromise;
}

function filterFixtureRowsByDate(rows: TickaEmissioniApiRow[], date: string) {
  return rows.filter((row) => normalizeDateValue((row as Record<string, unknown>).emissione_data) === date);
}

function filterFixtureRowsByRange(rows: TickaEmissioniApiRow[], from: string, to: string) {
  return rows.filter((row) => {
    const emissionDate = normalizeDateValue((row as Record<string, unknown>).emissione_data);
    return Boolean(emissionDate) && emissionDate >= from && emissionDate <= to;
  });
}

async function buildFixtureEmissioniResult(
  scope: { date: string } | { from: string; to: string },
  reason: "env-flag" | "live-empty"
): Promise<TickaEmissioniFetchResult> {
  const fixture = await loadTickaEmissioniFixtureRows();
  const rows =
    "date" in scope
      ? filterFixtureRowsByDate(fixture.rows, scope.date)
      : filterFixtureRowsByRange(fixture.rows, scope.from, scope.to);
  const sourceDate = "date" in scope ? scope.date : `${scope.from}:${scope.to}`;
  const normalized = normalizeTickaEmissioni(sourceDate, rows);

  console.log("[ticka-emissioni] using local fixture fallback", {
    reason,
    sourceMode: "fixture-fallback",
    fixtureFileUsed: fixture.filePath,
    scope,
    rawRowCount: rows.length,
    normalizedRowCount: normalized.rows.length,
  });
  if (isDebugTickaEnabled()) {
    console.log("[ticka-emissioni] fixture fallback sample", {
      fixtureFileUsed: fixture.filePath,
      sampleKeys: rows[0] ? Object.keys(rows[0]).slice(0, 20) : [],
      firstThreeRows: rows.slice(0, 3),
    });
  ...
  return {
    finalUrl: fixture.filePath ? `fixture://${path.relative(process.cwd(), fixture.filePath)}` : "fixture://missing",
    rows,
    normalized,
    sourceMode: "fixture-fallback",
    fixtureFileUsed: fixture.filePath,
